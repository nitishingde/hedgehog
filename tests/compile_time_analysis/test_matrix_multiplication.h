...
#ifndef HEDGEHOG_TEST_MATRIX_MULTIPLICATION_H
#define HEDGEHOG_TEST_MATRIX_MULTIPLICATION_H
...
#include "../data_structures/matrix_multiplication/state/output_state.h"
...
void testMatrixMultiplication() {
  ...
  using GraphType = hh::Graph<
      MatrixBlockData<MatrixType, 'c'>,
      MatrixData<MatrixType, 'a'>, MatrixData<MatrixType, 'b'>, MatrixData<MatrixType, 'c'>>;
  using InputStateManagerInputType = hh::StateManager<
      std::pair<
          std::shared_ptr<MatrixBlockData<MatrixType, 'a'>>,
          std::shared_ptr<MatrixBlockData<MatrixType, 'b'>>>,
      MatrixBlockData<MatrixType, 'a'>, MatrixBlockData<MatrixType, 'b'>>;

  constexpr hh::cx::CXNode<MatrixRowTraversalTask<MatrixType, 'a'>> nodeTraversalA("RowTraversalA");
  constexpr hh::cx::CXNode<MatrixColumnTraversalTask<MatrixType, 'b'>> nodeTraversalB("ColTraversalB");
  constexpr hh::cx::CXNode<MatrixRowTraversalTask<MatrixType, 'c'>> nodeTraversalC("RowTraversalC");
  constexpr hh::cx::CXNode<ProductTask<MatrixType>> nodeProductTask("ProductTask");
  constexpr hh::cx::CXNode<AdditionTask<MatrixType>> nodeAdditionTask("AdditionTask");
  constexpr hh::cx::CXNode<InputStateManagerInputType> nodeSMInput("SMInput");
  constexpr hh::cx::CXNode<PartialComputationStateManager<MatrixType>, MatrixBlockData<MatrixType, 'c'>>
      nodeSMComputation("SMComputation");
  constexpr hh::cx::CXNode<hh::StateManager<
      MatrixBlockData<MatrixType, 'c'>,
      MatrixBlockData<MatrixType, 'c'>>, MatrixBlockData<MatrixType, 'c'>>
      nodeSMOutput("SMOutput");

  constexpr auto defroster = [&]() {
    hh::cx::CXGraph<GraphType> staticGraph("Graph");

    staticGraph.input(nodeTraversalA);
    staticGraph.input(nodeTraversalB);
    staticGraph.input(nodeTraversalC);
    staticGraph.addEdge(nodeTraversalA, nodeSMInput);
    staticGraph.addEdge(nodeTraversalB, nodeSMInput);
    staticGraph.addEdge(nodeTraversalC, nodeSMComputation);
    staticGraph.addEdge(nodeSMInput, nodeProductTask);
    staticGraph.addEdge(nodeProductTask, nodeSMComputation);
    staticGraph.addEdge(nodeSMComputation, nodeAdditionTask);
    staticGraph.addEdge(nodeAdditionTask, nodeSMComputation);
    staticGraph.addEdge(nodeAdditionTask, nodeSMOutput);
    staticGraph.output(nodeSMOutput);

    auto cycleTest = hh::cx::test::CycleTest<GraphType>{};
    auto dataRaceTest = hh::cx::test::DataRaceTest<GraphType>{};
    staticGraph.addTest(cycleTest);
    staticGraph.addTest(dataRaceTest);

    return hh::cx::Defroster(staticGraph);
  }();
  ...
  if constexpr (defroster.isGraphValid()) {
    ...
    // Tasks
    auto taskTraversalA =
        std::make_shared<MatrixRowTraversalTask<MatrixType, 'a'>>();
    auto taskTraversalB =
        std::make_shared<MatrixColumnTraversalTask<MatrixType, 'b'>>();
    auto taskTraversalC =
        std::make_shared<MatrixRowTraversalTask<MatrixType, 'c'>>();
    auto productTask =
        std::make_shared<ProductTask<MatrixType>>(numberThreadProduct, p);
    auto additionTask =
        std::make_shared<AdditionTask<MatrixType>>(numberThreadAddition);

    // State
    auto stateInputBlock =
        std::make_shared<InputBlockState<MatrixType>>(nBlocks, mBlocks, pBlocks);
    ...
    auto stateManagerInputBlock =
        std::make_shared<InputStateManagerInputType>("Input State Manager", stateInputBlock);
    ...
    auto stateManagerOutputBlock =
        std::make_shared<hh::StateManager<
            MatrixBlockData<MatrixType, 'c'>,
            MatrixBlockData<MatrixType, 'c'>>>("Output State Manager", stateOutput);

    auto matrixMultiplicationGraph = defroster.convert(
        nodeTraversalA, taskTraversalA,
        nodeTraversalB, taskTraversalB,
        nodeTraversalC, taskTraversalC,
        nodeSMInput, stateManagerInputBlock,
        nodeSMComputation, stateManagerPartialComputation,
        nodeAdditionTask, additionTask,
        nodeProductTask, productTask,
        nodeSMOutput, stateManagerOutputBlock
    );
...
#endif //HEDGEHOG_TEST_MATRIX_MULTIPLICATION_H
