...
/// @details At initialization, the device is set to the task (cudaSetDevice), and hedgehog stream is created and bound to the
...
class AbstractCUDATask : public AbstractTask<TaskOutput, TaskInputs...> {
 ...
 public:
  ...
  /// @brief Initialize an AbstractCUDATask to bound it to hedgehog CUDA device, and do the peer access if enabled.
  ...
  /// @brief Accessor for peer access enabled for hedgehog specific device id
