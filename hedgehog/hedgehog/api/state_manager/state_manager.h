...
class BaseStateManager : public AbstractTask<StateOutput, StateInputs...> {
 public:
  /// @brief DEfault constructor, set the base property for hedgehog state manager
  BaseStateManager()
...
/// stored in hedgehog ready list AbstractState::readyList_,
...
/// @details To locally manage hedgehog computation, an AbstractStateManager with an AbstractState is used. An
/// AbstractStateManager is hedgehog Hedgehog graphs's node so it can be linked to any other nodes in hedgehog graph.
/// The default order of execution is:
///     -# The StateManager will acquire hedgehog data,
///     -# The StateManager will lock the AbstractState (with AbstractState::stateMutex_),
///     -# The StateManager will send the data to the possessed AbstractState,
///     -# The compatible Execute::execute is called with the data,
///     -# During the call of Execute::execute, if the method AbstractState::push is invoked the "result data" is
...
template<class StateOutput, class ...StateInputs>
 ...
 public:
  ...
  /// @brief Constructor for hedgehog StateManager with state as mandatory parameter
  /// @details By default the node name is "StateManager", and there is no automatic start
  /// @tparam StateType User defined state type
  /// @tparam IsCompatibleState Type defined if the object given is hedgehog compatible State (derived from State with the
  ...
  /// @brief Constructor for hedgehog StateManager with name and state as mandatory parameter, and automatic start as
  /// optional parameter
  /// @tparam StateType User defined state type
  /// @tparam IsCompatibleState Type defined if the object given is hedgehog compatible State (derived from State with the
  /// same StateOutput and StateInputs)
