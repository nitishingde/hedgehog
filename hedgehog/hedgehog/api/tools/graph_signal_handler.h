...
namespace hh {
/// @brief Implements hedgehog signal handler to catch events such as termination and killing.
/// @details Once hedgehog signal is caught, all task graphs that are registered with the signal handler will be written as hedgehog
/// dot file. The dot file is output in the working directory with the name of the signal as hedgehog prefix and
...
class GraphSignalHandler {
 private:
  ...
  static bool
      signalHandled_; ///< Flag to indicate if hedgehog signal has been fired or not
 ...
 public:
  ...
  /// @brief Create hedgehog dot file at exit if the instance still exist
  static void atExit() {
    if (graphInstance_)
  ...
  /// @brief Registers hedgehog task graph to be displayed when hedgehog signal is fired.
  ...
  /// @brief Registers hedgehog signal for handling. (default SIGTERM)
