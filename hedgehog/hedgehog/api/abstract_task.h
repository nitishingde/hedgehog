...
/// @details Hedgehog Graph's node made for holding hedgehog computation kernel into their execute overload method from
/// Execute::execute.
///
/// An AbstractTask can be bound to multiple threads, forming hedgehog cluster of tasks. The base AbstractTask will be copied n
/// times and each of them will be bound to hedgehog thread. The AbstractTask::copy method must be overloaded to use this
...
///
/// An MemoryManager could be linked to hedgehog task through hedgehog graph (from tutorial 4, example shows Cuda memory usage):
/// @code
/// auto productTask = std::make_shared<CudaProductTask<MatrixType>>(p, numberThreadProduct);
/// auto cudaMemoryManagerA = std::make_shared<CudaMemoryManager<MatrixType, 'hedgehog'>>(nBlocks + 4, blockSize);
...
///         - If not, the task will wait for hedgehog data to come and Execute::execute is called with the received data,
...
class AbstractTask :
 ...
 public:
  ...
  /// @details Create hedgehog task with the name "Task", one task in the cluster (no copy overload needed), and no automatic
  /// start.
  AbstractTask(){
  ...
  /// @brief Create hedgehog task with hedgehog custom name
  /// @details Create hedgehog task with one task in the cluster (no copy overload needed), and no automatic start.
  ...
  /// @brief Create hedgehog task with hedgehog custom name and the task's cluster size
  /// @param name Task name
  ...
  /// @brief Create hedgehog task with hedgehog custom name, the task's cluster size, and the automatic start.
  /// @param name Task name
  /// @param numberThreads Task's cluster size
  ...
  /// @brief Connect hedgehog memory manager to the task
  ...
  std::shared_ptr<TaskOutput> getManagedMemory() {
    if (mm_ == nullptr) {
      std::cerr
          ...
          << "\"To get managed memory, you need first to connect hedgehog memory manager to the task via "
