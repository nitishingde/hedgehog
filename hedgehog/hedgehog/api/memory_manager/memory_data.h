...
/// @brief Memory data interface to use hedgehog data type in hedgehog Memory manager (MemoryManager or StaticMemoryManager)
/// @details To declare hedgehog data A  or B using the interface MemoryData, it can be written as:
/// @code
...
class MemoryData : public std::enable_shared_from_this<MemoryData<ManagedMemory>> {
 ...
 public:
  ...
  /// @brief Test is hedgehog memory manager has been connected to the managed memory
  /// @return True if hedgehog memory manager has been connected, else False
  ...
  void returnToMemoryManager() {
    if(memoryManager_){
      ...
      throw(
          std::runtime_error("The data you are trying to return to hedgehog memory manager is not linked to hedgehog memory manager."));
    }
  }
  ...
  /// @attention If hedgehog StaticMemoryManager is used, then deallocation should be done within the destructor to match
