...
/// -# Provide hedgehog mechanism for data reuse (with recycling to clean to-be-reused pieces of memory).
///
...
/// these data are default constructed and stored into hedgehog Pool.
/// When constructed, hedgehog memory manager can be attached to hedgehog task with AbstractTask::connectMemoryManager().
///
...
///
/// The only pure virtual method is the copy method to duplicate hedgehog derived MemoryManager to different
...
/// };
///
/// // Task returning DynamicMemoryManagedData and getting data from hedgehog memory manager
...
class MemoryManager {
 ...
 public:
  ...
  /// @return Return hedgehog copy of this specialised MemoryManager
  ...
  /// @brief User-definable initialization step for hedgehog memory manager
...
/// @attention The existence of hedgehog memory manager into hedgehog Task should be checked before usage.
