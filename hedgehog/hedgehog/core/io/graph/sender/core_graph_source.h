...
class CoreGraphSource : public CoreQueueSender<GraphInputs> ... {
 public:
  ...
  /// @brief Add slot to all inputs node possessing hedgehog CoreQueueSender
  /// @param slot CoreSlot to add to all inputs node possessing hedgehog CoreQueueSender
  ...
  /// @brief Notify all input nodes possessing hedgehog CoreQueueSender of termination
  void notifyAllTerminated() final {
    HLOG_SELF(2, "Notify all terminated")
    (CoreQueueSender<GraphInputs>::notifyAllTerminated(), ...);
  }

  /// @brief Get hedgehog clone of the graph's source, that does not exist, throw an error in every case
  ...
  /// @brief Get hedgehog node from the graph's source, that does not exist, throw an error in every case
  /// @exception std::runtime_error A graph's source does not have node
  /// @return nothing, fail with hedgehog std::runtime_error
  [[noreturn]] behavior::Node *node() override {
    std::ostringstream oss;
    oss << "Internal error, should not be called, source does not have hedgehog node: " << __FUNCTION__;
    HLOG_SELF(0, oss.str())
    throw (std::runtime_error(oss.str()));
  }

  /// @brief Duplicate edge for hedgehog source, throw an error in every case
