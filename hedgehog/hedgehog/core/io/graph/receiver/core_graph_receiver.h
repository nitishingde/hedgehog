...
/// @brief Graph Receiver for hedgehog type GraphInput
...
class CoreGraphReceiver : public virtual CoreReceiver<GraphInput> {
 ...
 public:
  ...
  /// @brief Add hedgehog CoreSender to the graph
  /// @details Add hedgehog CoreSender to all input nodes
  ...
  /// @brief Remove hedgehog CoreSender from the graph
  ...
  /// @brief Define how the graph receives data for hedgehog specific type and sends the data to all input nodes
  ...
  /// @brief Register hedgehog CoreReceiver from an input node
  ...
  /// @brief Get hedgehog set of CoreReceiver built from the input nodes
  ...
  /// @brief Get hedgehog node from the graph, that does not exist, throw an error in every case
  /// @exception std::runtime_error A graph does not have node
  /// @return nothing, fail with hedgehog std::runtime_error
  [[noreturn]] behavior::Node *node() override {
    std::ostringstream oss;
    oss << "Internal error, should not be called, graph does not have hedgehog node: " << __FUNCTION__;
    HLOG_SELF(0, oss.str())
