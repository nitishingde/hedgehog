...
class CoreGraphSink : public CoreQueueMultiReceivers<GraphOutput> {
 public:
  ...
  /// @brief Get hedgehog node from the graph's sink, that does not exist, throw an error in every case
  ...
  [[noreturn]] behavior::Node *node() override {
    ...
    oss << "Internal error, should not be called, sink does not have hedgehog node: " << __FUNCTION__;
    HLOG_SELF(0, oss.str())
    throw (std::runtime_error(oss.str()));
  }

  /// @brief Get hedgehog clone of the graph's sink, that does not exist, throw an error in every case
  ...
  /// @brief Duplicate edge for hedgehog sink, throw an error in every case
