...
/// @brief Sender for nodes possessing hedgehog queue of data
...
class CoreQueueSender : public CoreSender<NodeOutput>, public virtual CoreQueueNotifier {
 ...
 public:
  ...
  /// @brief Add hedgehog receiver to the set of receivers
  ...
  void addReceiver(CoreReceiver<NodeOutput> *receiver) override {
    ...
    for (auto queueReceiver: receiver->receivers()) {
      if (auto r = dynamic_cast<CoreQueueReceiver<NodeOutput> *>(queueReceiver)) {
        ...
        oss
            << "Internal error, CoreQueueSender connected to hedgehog node which is not hedgehog CoreQueueReceiver: "
            << __FUNCTION__;
        HLOG_SELF(0, oss.str())
  ...
  /// @brief Remove hedgehog receiver from the set of receivers
  ...
  /// @brief Send hedgehog data to the list of destinations, and notify them
  ...
  /// @brief Special visit method for hedgehog CoreQueueSender, printing an edge
  ...
  /// @brief Copy the inner structure of hedgehog CoreQueueSender (destinations, and notifier)
  ...
  void duplicateEdge(CoreNode *duplicateNode,
    ...
    for (CoreQueueReceiver<NodeOutput> *originalReceiver : *(this->destinations())) {
      if (auto coreNode = dynamic_cast<CoreNode *>(originalReceiver)) {
        ...
        std::ostringstream oss;
        oss
            << "Internal error, The Receiver is not hedgehog CoreNode, copy failed "
            << __FUNCTION__;
        HLOG_SELF(0, oss.str())
        throw (std::runtime_error(oss.str()));
      }
 ...
 private:
  /// @brief Connect coreNodeSender to hedgehog coreNodeReceiver, connect for data transfer and signal handling
