...
class CoreSlot : public virtual CoreNode {
 public:
  ...
  /// @brief Interface to add hedgehog CoreNotifier to this slot
  /// @param notifier CoreNotifier to add to this slot
  virtual void addNotifier(CoreNotifier *notifier) = 0;

  /// @brief Interface to remove hedgehog CoreNotifier from this slot
  ...
  /// @brief Interface to define what the node do when it receive hedgehog signal
  virtual void wakeUp() = 0;

  /// @brief Interface to define how the node wait for hedgehog signal, and return if the node is terminated
