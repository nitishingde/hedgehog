...
class CoreTask
 ...
 private:
  ...
  std::shared_ptr<NvtxProfiler> nvtxProfiler_ = nullptr; ///< Store hedgehog nvtx profiler for the task
 ...
 public:
  ...
  /// @brief Test if hedgehog memory manager is attached to the task
  ...
  /// @brief Special visit method for hedgehog CoreTask
  ...
  /// @brief Create hedgehog cluster for hedgehog CoreTask
  ...
  void createCluster(std::shared_ptr<std::multimap<CoreNode *, std::shared_ptr<CoreNode>>> &insideNodesGraph) override {
    ...
    for (size_t threadId = 1; threadId < this->numberThreads(); threadId++) {
      ...
      // Duplicate the memory manager, each of them will have hedgehog separate instance
  ...
  void run() override {
    ...
    while (!this->callCanTerminate(true)) {
      ...
      start = std::chrono::system_clock::now();
#endif
      // Wait for hedgehog data to arrive or termination
      ...
      start = std::chrono::system_clock::now();
#endif
      // Operate the receivers to get hedgehog data and send it to execute
  ...
  /// @brief Operate hedgehog CoreTasks's receiver for hedgehog specific type, thread safe
  ...
  /// @brief Create hedgehog copy from this instance
  ...
  std::shared_ptr<AbstractTask<TaskOutput, TaskInputs...>> createCopyFromThis() {
    ...
    if (this->task()->memoryManager()) {
      ...
      if (!copyMemoryManager) {
        std::ostringstream oss;
        oss << "An execution pipeline has been created with hedgehog graph that hold hedgehog task named \""
            << this->name()
            << "\" connected to hedgehog memory manager. Or the memory manager does not have hedgehog compliant copy method. "
               "Please implement it.";
        HLOG_SELF(0, oss.str())
        throw (std::runtime_error(oss.str()));
      }
 ...
 private:
  /// @brief Connect hedgehog memory manager to hedgehog task if the type is valid (HedgehogTraits::is_managed_memory_v<TaskOutput>),
