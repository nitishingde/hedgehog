...
class CoreExecutionPipeline : public CoreTask<GraphOutput, GraphInputs...> {
 ...
 public:
    ...
    if (numberGraphs_ != deviceIds.size()) {
      ...
      oss
          << "The number of device Ids do not correspond to the number of coreGraph duplications you sent to the "
             "execution pipeline \""
          << name
          << "\". Even if you do not associate the graphs duplicates to hedgehog specific device, please set the deviceIds "
  ...
  /// @brief Get hedgehog device id, not possible for an execution pipeline, throw an error in every case
  /// @exception std::runtime_error An execution pipeline does not have hedgehog device id
  ...
  /// @brief Add hedgehog receiver to the execution pipeline, to all inside graphs
  ...
  void addReceiver(CoreReceiver<GraphOutput> *receiver) override {
    for (CoreReceiver<GraphOutput> *r : receiver->receivers()) {
      if (auto coreQueueReceiver = dynamic_cast<CoreQueueReceiver<GraphOutput> *>(r)) {
        ...
        std::ostringstream oss;
        oss << "Internal error, hedgehog receiver added to an execution pipeline is not hedgehog coreQueueReceiver : "
            << __FUNCTION__;
        HLOG_SELF(0, oss.str())
        throw (std::runtime_error(oss.str()));
      }
  ...
  /// @brief Add hedgehog slot to hedgehog execution pipeline, i.e. to all inside graphs
 ...
 private:
  ...
  /// @brief Connect hedgehog graph to the switch and register it
  ...
  /// @brief Connect hedgehog CoreReceiver to all output of hedgehog graph
