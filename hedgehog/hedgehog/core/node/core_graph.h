...
/// @details Internal representation of hedgehog graph in the hedgehog library. Hold nodes that's inside.
...
class CoreGraph : public CoreSender<GraphOutput>, public CoreGraphMultiReceivers<GraphInputs...> {
 ...
 public:
  ...
  /// @param scheduler Graph's scheduler, by default hedgehog DefaultScheduler
  ...
  /// @brief Clone hedgehog core graph calling the graph copy constructor
  ...
  [[nodiscard]] std::chrono::duration<double, std::micro> maxExecutionTime() const override {
    ...
    for (auto const &it : *(this->insideNodes())) {
      ...
      switch (core->type()) {
        ...
        case NodeType::ExecutionPipeline:temp = core->maxExecutionTime();
  ...
  [[nodiscard]] std::chrono::duration<double, std::micro> minExecutionTime() const override {
    ...
    for (auto const &it : *(this->insideNodes())) {
      ...
      switch (core->type()) {
        ...
        case NodeType::Graph:
        case NodeType::StateManager:
        case NodeType::ExecutionPipeline:temp = core->minExecutionTime();
          if (temp < ret) ret = temp;
          break;
        default:break;
      }
  ...
  [[nodiscard]] std::chrono::duration<double, std::micro> maxWaitTime() const override {
    ...
    for (auto const &it : *(this->insideNodes())) {
      ...
      switch (core->type()) {
        ...
        case NodeType::Graph:
        case NodeType::StateManager:
        case NodeType::ExecutionPipeline:temp = core->maxWaitTime();
          if (temp > ret) ret = temp;
          break;
        default:break;
      }
  ...
  [[nodiscard]] std::chrono::duration<double, std::micro> minWaitTime() const override {
    ...
    for (auto const &it : *(this->insideNodes())) {
      ...
      switch (core->type()) {
        ...
        case NodeType::Graph:
        case NodeType::StateManager:
        case NodeType::ExecutionPipeline:temp = core->minWaitTime();
          if (temp < ret) ret = temp;
          break;
        default:break;
      }
  ...
  void addEdge(std::shared_ptr<UserDefinedSender> from, std::shared_ptr<UserDefinedMultiReceiver> to) {
    using Output = typename UserDefinedSender::output_t;
    using Inputs = typename UserDefinedMultiReceiver::inputs_t;
    assert(from != nullptr && to != nullptr);
    static_assert(traits::Contains_v<Output, Inputs>, "The given Receiver cannot be linked to this Sender");
    if (this->isInside()) {
      std::ostringstream oss;
      oss << "You can not modify hedgehog graph that is connected inside another graph: " << __FUNCTION__;
      HLOG_SELF(0, oss.str())
      throw (std::runtime_error(oss.str()));
    }
    ...
    auto coreReceiver =
        dynamic_cast<CoreReceiver<Output> *>(std::static_pointer_cast<behavior::Node>(to)->core().get());

    if (from->core().get() == this || to->core().get() == this) {
      std::ostringstream oss;
      oss << "You can not connect hedgehog graph to itself: " << __FUNCTION__;
      HLOG_SELF(0, oss.str())
      throw (std::runtime_error(oss.str()));
    }
  ...
  /// @brief Set hedgehog node as input for the graph
  ...
  template<HedgehogMultiReceiver UserDefinedMultiReceiver>
  void input(std::shared_ptr<UserDefinedMultiReceiver> inputNode) {
    using InputsMR = typename UserDefinedMultiReceiver::inputs_t;
    static_assert(
        traits::is_included_v<InputsMR, typename behavior::MultiReceivers<GraphInputs...>::inputs_t>,
    ...
    if (auto inputCoreNode =
      ...
      std::ostringstream oss;
      oss << "The node " << inputCoreNode->name() << " is not hedgehog multi receiver: " << __FUNCTION__;
      HLOG_SELF(0, oss.str())
      throw (std::runtime_error(oss.str()));
    }
  ...
  void output(std::shared_ptr<behavior::Sender<GraphOutput>> outputNode) {
    ...
    if (auto outputCoreNode = dynamic_cast<CoreSender<GraphOutput> *>(outputNode->core().get())) {
      ...
      std::ostringstream oss;
      oss << "Internal error, the output node is not hedgehog valid CoreSender: " << __FUNCTION__;
      HLOG_SELF(0, oss.str())
      throw (std::runtime_error(oss.str()));
    }
  ...
  void setInside() override {
    ...
    for (CoreNode *inputNode: *(this->inputsCoreNodes_)) {
      if (auto coreSlot = dynamic_cast<CoreSlot *>(inputNode)) {
        ...
        std::ostringstream oss;
        oss << "Internal error, the input node is not hedgehog slot, when graph is set inside : " << __FUNCTION__;
        HLOG_SELF(0, oss.str())
        throw (std::runtime_error(oss.str()));
      }
    }
...
//                                (endExecutionTimeStamp - this->startExecutionTimeStamp()));
  ...
  /// @attention If finishPushingData is not called there is hedgehog risk of deadlock
  ...
  /// @brief Special visit method for hedgehog CoreGraph
  ...
  //Virtual functions
  //Sender
  /// @brief Add hedgehog receiver to the graph, i.e, add hedgehog receiver to all output nodes.
  ...
  /// @brief Remove hedgehog receiver from the graph, i.e, remove hedgehog receiver from all output nodes.
  ...
  /// @brief Send hedgehog data and notify receivers, not possible for hedgehog graph, throws an error in every case
  ...
  void sendAndNotify([[maybe_unused]]std::shared_ptr<GraphOutput> ptr) override {
    std::ostringstream oss;
    oss << "Internal error, hedgehog graph do not send data: " << __FUNCTION__;
    HLOG_SELF(0, oss.str())
    throw (std::runtime_error(oss.str()));
  }

  //Notifier
  /// @brief Add hedgehog slot to hedgehog graph, i.e, to all output nodes
  ...
  /// @brief Remove hedgehog slot from hedgehog graph, i.e, from all output nodes
  ...
  /// @brief Notify termination to all connected nodes, not possible for hedgehog graph, throw an error in every case
  /// @exception std::runtime_error A graph do not send data
  void notifyAllTerminated() override {
    std::ostringstream oss;
    oss << "Internal error, hedgehog graph  do not notify nodes: " << __FUNCTION__;
    HLOG_SELF(0, oss.str())
    throw (std::runtime_error(oss.str()));
  }

  /// @brief Add hedgehog notifier to the graph, ie, to all input nodes
  ...
  void addNotifier(CoreNotifier *notifier) override {
    ...
    for (CoreNode *inputNode: *(this->inputsCoreNodes_)) {
      if (auto coreSlot = dynamic_cast<CoreSlot *>(inputNode)) {
        ...
        std::ostringstream oss;
        oss << "Internal error, A graph's input node is not hedgehog slot: " << __FUNCTION__;
        HLOG_SELF(0, oss.str())
        throw (std::runtime_error(oss.str()));
      }
    }
  ...
  /// @brief Remove hedgehog notifier from the graph, ie, from all input nodes
  ...
  /// @brief Return the number of input nodes connected, hedgehog graph should not have such hedgehog connection, throws in every case
  /// @return nothing, throw an error
  ...
  /// @brief Wake up hedgehog graph, wake up all input nodes
  void wakeUp() override {
    ...
    for (CoreNode *inputNode: *(this->inputsCoreNodes_)) {
      if (auto coreSlot = dynamic_cast<CoreSlot *>(inputNode)) {
        ...
        std::ostringstream oss;
        oss << "Internal error, A graph's input is not hedgehog core slot: " << __FUNCTION__;
        HLOG_SELF(0, oss.str())
        throw (std::runtime_error(oss.str()));
      }
    }
  ...
  bool waitForNotification() override {
    std::ostringstream oss;
    oss << "Internal error, hedgehog graph is not connected to input nodes, so do not wait for notification: "
        << __FUNCTION__;
    HLOG_SELF(0, oss.str())
    throw (std::runtime_error(oss.str()));
  }
 ...
 private:
  /// @brief Register hedgehog node inside the graph
  ...
  void duplicateInsideNodes(CoreGraph<GraphOutput, GraphInputs...> const &rhs) {
    ...
    for (CoreSender<GraphOutput> *originalOutputNode : *(rhs.outputCoreNodes())) {
      ...
      if (auto outputCoreNode = dynamic_cast<CoreSender<GraphOutput> *>(shOutputCoreNode.get())) {
        ...
        std::ostringstream oss;
        oss << "Internal error, the output node is not hedgehog sender: " << __FUNCTION__;
        HLOG_SELF(0, oss.str())
        throw (std::runtime_error(oss.str()));
      }
    }
  ...
  void duplicateInputNodes(CoreReceiver<InputNodeType> *inputCoreNode) {
    if (inputCoreNode) {
      ...
      if (auto coreSlot = dynamic_cast<CoreSlot *>(inputCoreNode)) {
        ...
        std::ostringstream oss;
        oss << "Internal error, the inputCoreNode is not hedgehog CoreSlot: " << __FUNCTION__;
        HLOG_SELF(0, oss.str())
        throw (std::runtime_error(oss.str()));
      }
      std::static_pointer_cast<CoreQueueSender<InputNodeType>>(this->source_)->addReceiver(dynamic_cast<CoreReceiver<
          InputNodeType> *>(inputCoreNode));
      dynamic_cast<CoreReceiver<InputNodeType> *>(inputCoreNode)
          ->addSender(static_cast<CoreSender<InputNodeType> *>(this->source_.get()));
    }
  }

  /// @brief Specialized method if the input node is in hedgehog cluster
