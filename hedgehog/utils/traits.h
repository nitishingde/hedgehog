...
#ifndef HEDGEHOG_CX_TRAITS_H
#define HEDGEHOG_CX_TRAITS_H
...
namespace hh {
  ...
  namespace traits {
    /// @brief Check if all parameters in hedgehog parameters pack are unique, default case
    /// @tparam ... Default list of templates
    template<typename...>
    inline constexpr auto isUnique = std::true_type{};

    /// @brief Check if all parameters in hedgehog parameters pack are unique, looping through templates
    ...
    /// @brief Test correctness of type given to hedgehog memory manager
    ...
    /// @brief Check if hedgehog template T is in Template pack Ts
    ...
    /// @brief Check if hedgehog template T is in tuple containing template pack Ts
    /// @tparam T Type to test
    /// @tparam Ts Parameter pack
    template<class T, class... Ts>
    ...
    /// @brief Check if hedgehog tuple of types T1 is included in hedgehog tuple of type T2, all type in T1 are in T2
    /// @tparam T1 Tuple of types that should be in type T2
    /// @tparam T2 Tuple of types that should contain all type of T1
    /// @tparam Is List of index of tuple T1
    ...
    /// @brief Create hedgehog std::string_view containing hedgehog full type name of T
    /// @tparam T Type to get the name from
    /// @return std::string_view containing hedgehog full type name of T
    template<typename T>
